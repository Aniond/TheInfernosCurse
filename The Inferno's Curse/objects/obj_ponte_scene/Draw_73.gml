...
// =============================================================================
// Drawn AFTER all depth-sorted world drawing (under the GUI): two terracotta
// roof runs span the corridor — the bridge reads as a covered merchant
// gallery. The central plaza and both landings stay OPEN to the sky (the
// Arno viewing points). The segment over Benedetto fades to ~0.45 alpha so
// the corridor interior stays readable — the classic SNES roof cutaway.
// =============================================================================
...
var _roof = asset_get_index("spr_ponte_roof_tile");
var _has  = (_roof >= 0 && asset_get_type("spr_ponte_roof_tile") == asset_sprite);
...
for (var _i = 0; _i < array_length(_segs); _i++) {
    ...
    if (_has) {
        for (var _ry = _s[1]; _ry < _s[3]; _ry += 64)
            for (var _rx = _s[0]; _rx < _s[2]; _rx += 64)
                draw_sprite_part_ext(_roof, 0, 0, 0,
                    min(64, _s[2] - _rx), min(64, _s[3] - _ry), _rx, _ry, 1, 1, _tint, canopy_a[_i]);
    } else {
        draw_set_color(merge_color(make_color_rgb(176, 84, 58), make_color_rgb(70, 56, 60), _corr * 0.55));
